...
class Terrain(Scene2D):
    ...
    def task1(self):
        ...
        contours, hierarchy = cv2.findContours(skeleton_uint8, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) 
        half_contours = contours[::2]

        #  correct the hierarchy 
        corrected_hierarchy = hierarchy[0]  # only keep hierarchy entries for kept contours
        final_hierarchy=[]
        for i,h in enumerate(corrected_hierarchy):
            if (i%2==0):
                # print(h)
                if(h[0]%2==0):
                    h[0]+=1
                if(h[1]%2==0):
                    h[1]-=1
                final_hierarchy.append(h)
            else:
                if(h[0]==h[1]==h[2]==-1):
                    h[3]+=1
                    final_hierarchy.append(h)

        self.hierarchy = np.array(final_hierarchy).squeeze().tolist()
        print("final_hierarchy",self.hierarchy)
        ...
        for i, c in enumerate(half_contours):
            ...
            self.raw_contours.append(pointset)
            count += 1
        print("number of points in all contour",self.raw_contours)   
        return self.raw_contours
    
    def count_children(self, idx):
        count = 0
        child = self.hierarchy[idx][2]  # First_Child
        while child != -1:
            if child >= len(self.hierarchy):
                # Invalid index! Stop or handle the error
                break
            count += 1
            child = self.hierarchy[child][0]  # Next sibling
        return count
    ...
    def categorize_contours(self):
        ...
        for i in range(len(self.raw_contours)):
            c = self.count_children( i)
            print("children counter",c)
            if c == 1:
                peaks.append(i)
            elif c == 2:
                simple.append(i)
    ...
    def task2(self):
        contours = self.raw_contours
        peaks, simple, complex = self.categorize_contours()
        print(simple)
        print(peaks)

        for p in peaks:
            print("peaks",contours[p])
        for s in simple:
            print("simple",contours[s])
        for c in complex :
            print("complex",contours[c])
        # self.triangulate_between(contours[simple],contours[simple+1])
